...
type BurialGrounds struct {
	ctx *context.Status
}

func NewBurialGrounds() *BurialGrounds {
	return &BurialGrounds{
		ctx: context.Get(),
	}
}

func (a BurialGrounds) Name() string {
	return string(config.BloodMoorRun)
}

func (a BurialGrounds) CheckConditions(parameters *RunParameters) SequencerResult {
	// You can add any checks here if needed
	return SequencerOk
}

func (a BurialGrounds) Run(parameters *RunParameters) error {
